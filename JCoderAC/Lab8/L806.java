...
public class L806 {
    ...
    private static final AVLTree peopleAVLTree=new AVLTree();
    private static final AVLTree animalsAVLTree=new AVLTree();
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            int op=nextInt();
            long p=nextLong();
            answer+=getAnswer(op,p);
        }

        fastWriter.println(answer);
        fastWriter.close();
    }
    private static long getAnswer(int op,long p){
        long answer=0;
        long selected=0;
        if(op==0){
            long predecessor=peopleAVLTree.getPredecessor(p);
            long successor=peopleAVLTree.getSuccessor(p);

            if(predecessor==AVLTree.NO_PREDECESSOR&&successor==AVLTree.NO_SUCCESSOR){
                animalsAVLTree.addValue(p);
            }else {
                long difference_pre=Math.abs(predecessor-p);
                long difference_suc=Math.abs(successor-p);
                selected=(difference_pre<=difference_suc)
                        ?predecessor
                        :successor;

                peopleAVLTree.removeValue(selected);
                answer=Math.abs(selected- p);
            }
        }else {
            long predecessor=animalsAVLTree.getPredecessor(p);
            long successor=animalsAVLTree.getSuccessor(p);

            if(predecessor==AVLTree.NO_PREDECESSOR&&successor==AVLTree.NO_SUCCESSOR){
                peopleAVLTree.addValue(p);
            }else {
                long difference_pre=Math.abs(predecessor-p);
                long difference_suc=Math.abs(successor-p);
                selected=(difference_pre<=difference_suc)
                ...
                animalsAVLTree.removeValue(selected);
                answer=Math.abs(selected- p);
            }
        }
    ...
    private static class AVLTree{
        ...
        static final long NO_PREDECESSOR=(long)-1e15;
        static final long NO_SUCCESSOR=(long)1e15;

        public AVLTree(){
            this.root=null;
            this.size=0;
        }
        public long getPredecessor(long value){
            this.predecessor=null;
            setPredecessor(root,value);
            return this.predecessor==null?NO_PREDECESSOR:this.predecessor.value;
        }

        public long getSuccessor(long value){
            this.successor=null;
            setSuccessor(root,value);
            return this.successor==null?NO_SUCCESSOR:this.successor.value;
        }
